 ...
 */
const { wrapEmail, sectionHeading, detailsCard, ctaButton, greeting, note, formatDate, tokens, iconCalendar, iconMoon, iconKey, iconClipboard, iconPin } = require('./emailLayout');
...
function buildCheckInRequestEmail({ guestName, checkInDate, checkOutDate, nights, checkInUrl, locale = 'en' }) {
    ...
    const t = {
        en: {
            ...
            important: `${iconKey()} <strong>Important:</strong> Access details — including door codes, WiFi credentials, and arrival instructions — will only be provided once all guests have been registered through this form. Please complete it at your earliest convenience to ensure a smooth arrival.`,
            noteText: `${iconPin()} You can update the information anytime by reopening the same link.`,
        ...
        cs: {
            ...
            important: `${iconKey()} <strong>Důležité:</strong> Přístupové údaje — včetně kódů ke dveřím, přihlašovacích údajů k WiFi a pokynů k příjezdu — budou poskytnuty až po registraci všech hostů prostřednictvím tohoto formuláře. Prosím vyplňte jej co nejdříve, aby váš příjezd proběhl hladce.`,
            noteText: `${iconPin()} Údaje můžete kdykoli upravit otevřením stejného odkazu.`,
            subject: 'Online Check-In — Paraíso',
        },
        es: {
            ...
            important: `${iconKey()} <strong>Importante:</strong> Los datos de acceso — incluyendo códigos de entrada, credenciales WiFi e instrucciones de llegada — solo se proporcionarán una vez que todos los huéspedes hayan sido registrados a través de este formulario. Por favor, complétalo lo antes posible para garantizar una llegada sin contratiempos.`,
            noteText: `${iconPin()} Puedes actualizar la información en cualquier momento reabriendo el mismo enlace.`,
    ...
    const content = [
        ...
        ctaButton({ label: `${iconClipboard(tokens.white)} ${l.ctaLabel}`, url: checkInUrl }),
        importantBox,
        note(l.noteText),
    ].join('\n');
